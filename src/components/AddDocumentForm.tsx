...
const AddDocumentForm: React.FC<AddDocumentFormProps> = ({ onBack, onSuccess, editingDocument }) => {
  ...
  const handleDeleteEntity = (entityId: string) => {
    ...
    if (entity) {
      setItemToDelete({ id: entity.id, name: entity.name });
      setIsDeleteEntityDialogOpen(true);
    }
  };

  const handleDeleteDocumentType = (typeId: string) => {
    const type = customDocumentTypes.find(t => t.id === typeId);
    if (type) {
  ...
  const confirmDeleteEntity = () => {
    if (itemToDelete) {
      EntityService.deleteEntity(itemToDelete.id);
      loadEntities();
      if (formData.entityId === itemToDelete.id) {
  ...
  const confirmDeleteDocumentType = () => {
    if (itemToDelete) {
      CustomDocumentService.deleteCustomDocumentType(itemToDelete.id);
      loadCustomDocumentTypes();
      if (formData.type === itemToDelete.id) {
        setFormData({ ...formData, type: '' as DocumentType });
      }
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              >
                <SelectTrigger className="mobile-tap bg-card-bg border-gray-300">
                  <SelectValue placeholder="Select entity" />
                </SelectTrigger>
                <SelectContent className="bg-card-bg border-gray-300 z-50">
                  {entities.map((entity) => (
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                required
                className="mobile-tap bg-card-bg border-gray-300"
              />
            </div>
                  ...
                  {getAllDocumentOptions().map((option) => (
                    <SelectItem key={option.value} value={option.value}>
                      <div className="flex items-center justify-between w-full">
                        ...
                        {customDocumentTypes.some(t => t.id === option.value) && (
                          <Button
                            variant="ghost"
                            size="icon"
                            className="h-6 w-6"
                            onClick={(e) => {
                              e.stopPropagation();
                              handleDeleteDocumentType(option.value);
                            }}
                          >
                            <Trash2 className="h-3 w-3 text-red-500" />
                          </Button>
      ...
      <AlertDialog open={isDeleteEntityDialogOpen} onOpenChange={setIsDeleteEntityDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete the entity "{itemToDelete?.name}". This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <AlertDialog open={isDeleteTypeDialogOpen} onOpenChange={setIsDeleteTypeDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Are you sure?</AlertDialogTitle>
            <AlertDialogDescription>
              This will permanently delete the document type "{itemToDelete?.name}". This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setItemToDelete(null)}>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={confirmDeleteDocumentType}>Delete</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
